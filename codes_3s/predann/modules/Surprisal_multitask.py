# coding: utf-8
"""
Multitask mode for joint Song ID and Surprisal prediction.
Song ID classification is used jointly to improve the prediction accuracy of Surprisal prediction.
"""
...
class JEPA_Multitask(LightningModule):
    """
    Joint Surprisal and Song ID classification from 3-second EEG signals.

    Input:
        eeg: [B, 128, 3, 125]
    Output:
        Song ID logits: (B, 10)
        Surprisal logits: (B, 150, 128)
    """
    ...
    def forward(self, batch):
        eeg, label, surp_id, surp_raw = batch
        eeg = eeg.view(eeg.size(0), 128, 3, 125)
        surprisal_logit, cls_token, mask_pos = self.emenc(eeg, Surp_or_Entropy_raw=surp_raw)
        return surprisal_logit, cls_token, mask_pos

    def training_step(self, batch, _):
        eeg, label, surp_id, surp_raw = batch
        surprisal_logit, cls_token, mask_pos = self.forward(batch)

        masked_logits = surprisal_logit[mask_pos]
        masked_targets = surp_id[mask_pos]
        surprisal_ce_loss = self.ce_loss(masked_logits, masked_targets)

        song_id_logit = self.projector1(self.norm(cls_token))
        song_id_ce_loss = self.ce_loss(song_id_logit, label)

        total_loss = 0.1 * surprisal_ce_loss + 1.0 * song_id_ce_loss
        song_id_acc = (song_id_logit.argmax(dim=1) == label).float().mean()

        self.log("Surprisal_CE_loss/train", surprisal_ce_loss, on_epoch=True, prog_bar=False)
    ...
    def validation_step(self, batch, _):
        ...
        self.log("Surprisal_CE_loss/valid", surprisal_ce_loss, on_epoch=True, prog_bar=False)
