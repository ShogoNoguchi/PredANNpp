...
"""Pre-training encoder-decoder model for MuQ prediction at 40ms intervals."""
...
class Comp1EDPretrain(nn.Module):
    ...
    Decoder G : [F(EEG) ‖ mask×75] → 2×Block
    Output: (B,75,128)
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.time40_emb = nn.Parameter(torch.zeros(75, embed_dim))
        self.mask_token = nn.Parameter(torch.zeros(1, embed_dim))
        self.music_feat_proj = nn.Linear(1024, embed_dim)
        ...
        trunc_normal_(self.time40_emb, std=0.02)
    ...
    def forward(self, eeg_raw: torch.Tensor, muq_raw: torch.Tensor = None, ch_name_list=None):
        """
        eeg_raw: (B,128,3,125)
        muq_raw: optional (B,75,1024) 
        ...
        # 4. Mask tokens (learnable + 40 ms pos-emb)
        time40_emb = self.time40_emb.unsqueeze(0).expand(B, -1, -1)  # (B,75,512)
        mask_tok = self.mask_token.unsqueeze(0).expand(B, 75, -1)    # (B,75,512)

        if muq_raw is None:
            raise ValueError("muq_raw is required but was None")

        # 5. Replace a portion of MuQ embeddings (randomly select and replace 50%)
        num_tokens = muq_raw.size(1)               # 75
        num_replace = num_tokens // 2              # 50%

        replace_mask = torch.zeros((B, num_tokens), dtype=torch.bool, device=muq_raw.device)
        random_scores = torch.rand(B, num_tokens, device=muq_raw.device)
        topk_indices = random_scores.topk(num_replace, dim=1).indices
        replace_mask.scatter_(1, topk_indices, True)

        proj_feat = self.music_feat_proj(muq_raw)  # (B,75,512)
        final_tok = torch.where(replace_mask.unsqueeze(-1), proj_feat, mask_tok) + time40_emb

        h_dec = self.decoder(torch.cat([h_enc, final_tok], dim=1))   # (B,384+1 +75,512)
        h_mask = h_dec[:, -75:, :]                                   # (B,75,512)
        out = self.proj_out(h_mask)                                  # (B,75,128)

        mask_pos = ~replace_mask                                      # (B,75) positions of mask_token
...
# ----------------------------------------------------------------------
@register_model
def ms40_comp1_pretrain_ed_2layer_512(pretrained: bool = False, init_ckpt: str = "", **kwargs):
    """
    timm.create_model(
    ...
    if pretrained and init_ckpt:
        ...
        print(f"[ms40_comp1_pretrain_ed_2layer_512] loaded from {init_ckpt}")
    return model
